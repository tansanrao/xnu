...
class BCM2711Serial : public IOService
{
	OSDeclareDefaultStructors(BCM2711Serial);
	bool installed;
...
OSDefineMetaClassAndStructors(BCM2711Serial, IOService)

bool
BCM2711Serial::start(IOService *provider)
{
	...
	if (provider->enableInterrupt(0) != kIOReturnSuccess ||
		...
		installed = false;
		IOLog("BCM2711Serial: RX IRQ unavailable, using console polling\n");
	} else {
		IOLog("BCM2711Serial: PL011 RX/timeout IRQ ready\n");
	}
	registerService();
	return true;
}
...
void
BCM2711Serial::interrupt(__unused OSObject *target, __unused void *refCon,
...
void
BCM2711Serial::stop(IOService *provider)
{
	if (installed) {
