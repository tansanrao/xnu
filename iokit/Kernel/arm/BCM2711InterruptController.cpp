...
OSDefineMetaClassAndStructors(BCM2711InterruptController, IOInterruptController)

bool
BCM2711InterruptController::start(IOService *provider)
{
	if (!super::start(provider)) {
		return false;
	}
	sourceCount = bcm2711_gic_intid_count() + 16 * BCM2711_GIC_MAX_CPUS;
	...
	bcm2711_gic_set_dispatch(dispatch, this);
	...
	registerService();
	IOLog("BCM2711GIC: registered %u INTIDs, per-CPU PPI banks\n",
	    bcm2711_gic_intid_count());
	return true;
}

void
BCM2711InterruptController::free(void)
{
	if (reportCall) {
...
bool
BCM2711InterruptController::decode(IOService *nub, int source, bcm2711_irq_spec *spec)
{
	...
	return spec->intid >= 16 && spec->intid < bcm2711_gic_intid_count() &&
	       spec->intid != BCM2711_GIC_TIMER &&
	       (spec->type == kIOInterruptTypeEdge || spec->type == kIOInterruptTypeLevel) &&
	       spec->priority <= 0xe0 && (spec->priority & 0xf) == 0 &&
	       spec->cpu < ml_get_cpu_count() && spec->cpu < BCM2711_GIC_MAX_CPUS;
}

BCM2711InterruptController::Source *
BCM2711InterruptController::lookup(const bcm2711_irq_spec &spec)
{
	if (spec.intid < 32) {
		 ...
		 */
		if (spec.cpu != bcm2711_gic_current_cpu()) {
			return nullptr;
		}
		return &sources[bcm2711_gic_intid_count() + spec.cpu * 16 + spec.intid - 16];
	}
	return &sources[spec.intid];
}

IOReturn
BCM2711InterruptController::registerInterrupt(IOService *nub, int source,
    void *target, IOInterruptHandler handler, void *refCon)
{
	...
	} else if (!bcm2711_gic_configure(&spec)) {
...
IOReturn
BCM2711InterruptController::unregisterInterrupt(IOService *nub, int source)
{
	bcm2711_irq_spec spec;
	...
	bcm2711_gic_mask(spec.intid);
	bcm2711_gic_set_pending(spec.intid, false);
...
IOReturn
BCM2711InterruptController::change(IOService *nub, int source, bool enabled)
{
	bcm2711_irq_spec spec;
	if (!decode(nub, source, &spec)) {
		return kIOReturnBadArgument;
...
IOReturn BCM2711InterruptController::enableInterrupt(IOService *nub, int source)
{
	return change(nub, source, true);
}
IOReturn BCM2711InterruptController::disableInterrupt(IOService *nub, int source)
{
	return change(nub, source, false);
}

IOReturn
BCM2711InterruptController::getInterruptType(IOService *nub, int source, int *type)
{
	bcm2711_irq_spec spec;
...
IOReturn
BCM2711InterruptController::setPending(IOService *nub, int source, bool pending)
{
	bcm2711_irq_spec spec;
	if (!decode(nub, source, &spec)) {
		return kIOReturnBadArgument;
	...
	if (slot && slot->vector.nub == nub && slot->vector.source == source && !slot->removing) {
		bcm2711_gic_set_pending(spec.intid, pending);
...
IOReturn
BCM2711InterruptController::getPending(IOService *nub, int source, bool *pending)
{
	bcm2711_irq_spec spec;
	...
	IOReturn result = kIOReturnNoInterrupt;
	if (slot && slot->vector.nub == nub && slot->vector.source == source && !slot->removing) {
		*pending = bcm2711_gic_is_pending(spec.intid);
		result = kIOReturnSuccess;
	}
	IOSimpleLockUnlockEnableInterrupt(controllerLock, state);
	return result;
...
IOReturn BCM2711InterruptController::causeInterrupt(IOService *nub, int source)
{
	return setPending(nub, source, true);
}

bool BCM2711InterruptController::dispatch(void *target, uint32_t intid)
{
	return static_cast<BCM2711InterruptController *>(target)->dispatchInterrupt(intid);
}

bool
BCM2711InterruptController::dispatchInterrupt(uint32_t intid)
{
	IOInterruptState state = IOSimpleLockLockDisableInterrupt(controllerLock);
	bcm2711_irq_spec spec = { intid, 0, 0, bcm2711_gic_current_cpu() };
	Source *slot = lookup(spec);
	IOInterruptVector &v = slot->vector;
	...
	if (!v.interruptDisabledSoft && !slot->removing) {
		...
		bcm2711_gic_unmask(intid);
	}
	IOSimpleLockUnlockEnableInterrupt(controllerLock, state);
	return true;
}
void
BCM2711InterruptController::reportUnowned(thread_call_param_t target,
    __unused thread_call_param_t unused)
{
	auto controller = static_cast<BCM2711InterruptController *>(target);
	for (uint32_t cpu = 0; cpu < BCM2711_GIC_MAX_CPUS; cpu++) {
		for (uint32_t word = 0; word < 32; word++) {
			...
			while (bits) {
				...
				bits &= bits - 1;
				IOLog("BCM2711GIC: masked unowned INTID %u on CPU %u\n", word * 32 + bit, cpu);
			}
		}
	}
}
