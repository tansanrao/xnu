...
void bcm2712_gic_set_ipi_handler(ipi_handler_t handler);
...
static _Atomic(bcm2712_irq_dispatch_t) gic_dispatch;
...
static _Atomic uint32_t gic_targets[BCM2712_GIC_MAX_CPUS];
static _Atomic uint64_t gic_timer_count[BCM2712_GIC_MAX_CPUS];
static _Atomic uint64_t gic_ipi_count[BCM2712_GIC_MAX_CPUS];
...
uint32_t bcm2712_gic_intid_count(void) { return gic_intids; }
uint32_t bcm2712_gic_current_cpu(void) { return getCpuDatap()->cpu_number; }
void bcm2712_gic_mask(uint32_t intid) { gic_bit_write(GICD_ICENABLER, intid); }
void bcm2712_gic_unmask(uint32_t intid) { gic_bit_write(GICD_ISENABLER, intid); }

void
bcm2712_gic_set_pending(uint32_t intid, bool pending)
{
	gic_bit_write(pending ? GICD_ISPENDR : GICD_ICPENDR, intid);
}

bool
bcm2712_gic_is_pending(uint32_t intid)
{
...
bool
bcm2712_gic_is_enabled(uint32_t intid)
{
...
bool
bcm2712_gic_configure(const struct bcm2712_irq_spec *spec)
{
...
void
bcm2712_gic_set_dispatch(bcm2712_irq_dispatch_t dispatch, void *target)
{
	if (os_atomic_load(&gic_dispatch, acquire) != NULL) {
		panic("BCM2712: duplicate GIC controller");
	}
...
static void
bcm2712_gic_handle_irq(__unused void *target, __unused void *refCon,
    __unused void *nub, __unused int source)
{
	...
	if (intid >= BCM2712_GIC_MAX_INTIDS || intid >= gic_intids) {
		/* 1022 is a wrong-group response, not an innocuous spurious IRQ. */
		panic("BCM2712: invalid/special GIC IAR 0x%x on CPU %u", iar, cpu);
	}
	if (intid == BCM2712_GIC_TIMER) {
		os_atomic_inc(&gic_timer_count[cpu], relaxed);
		getCpuDatap()->cpu_decrementer = (uint32_t)-1;
		rtclock_intr(TRUE);
	} else if (intid == BCM2712_GIC_IPI) {
		ipi_handler_t handler = os_atomic_load(&gic_ipi_handler, acquire);
		if (handler == NULL) {
			panic("BCM2712: IPI arrived before handler registration");
		}
		os_atomic_inc(&gic_ipi_count[cpu], relaxed);
		os_atomic_or(&gic_ipi_seen, 1U << cpu, relaxed);
		handler();
	} else {
		bcm2712_irq_dispatch_t dispatch = os_atomic_load(&gic_dispatch, acquire);
		if (intid < 16) {
			panic("BCM2712: unowned SGI %u on CPU %u", intid, cpu);
		}
		if (dispatch == NULL || !dispatch(gic_dispatch_target, intid)) {
			...
			if (dispatch == NULL) {
				bcm2712_gic_mask(intid);
				bcm2712_gic_set_pending(intid, false);
				panic("BCM2712: unowned INTID %u before IOKit dispatch", intid);
...
uint32_t
bcm2712_gic_ipi_seen_mask(void)
{
	return os_atomic_load(&gic_ipi_seen, relaxed);
}

void
bcm2712_gic_get_stats(struct bcm2712_gic_stats *stats)
{
	for (unsigned int cpu = 0; cpu < BCM2712_GIC_MAX_CPUS; cpu++) {
...
void
bcm2712_gic_set_ipi_handler(ipi_handler_t handler)
{
	if (!os_atomic_cmpxchg(&gic_ipi_handler, NULL, handler, release) &&
	    os_atomic_load(&gic_ipi_handler, acquire) != handler) {
		panic("BCM2712: inconsistent IPI handlers");
	}
}

void
bcm2712_gic_validate_cpu(void)
{
	uint32_t cpu = bcm2712_gic_current_cpu();
	...
	for (unsigned int other = 0; other < BCM2712_GIC_MAX_CPUS; other++) {
		if (other != cpu && os_atomic_load(&gic_targets[other], acquire) == target) {
			panic("BCM2712: duplicate GIC CPU target 0x%x", target);
		}
	}
...
void
bcm2712_gic_init_cpu(void)
{
	uint32_t cpu = bcm2712_gic_current_cpu();
	if (cpu >= BCM2712_GIC_MAX_CPUS) {
		panic("BCM2712: GIC CPU %u out of range", cpu);
	}
	...
	/* BL31 owns security assignment. In the non-secure register view these
	 * sources must already appear as Group 1.
	 */
	...
	*(volatile uint8_t *)(gic_distributor_base + GICD_IPRIORITYR + BCM2712_GIC_TIMER) = 0x60;
	...
	gic_write32(gic_distributor_base, GICD_ISENABLER,
	    (1U << BCM2712_GIC_IPI) | (1U << BCM2712_GIC_TIMER));
	...
	/* Enable the non-secure Group 1 CPU interface. EOImodeNS remains zero,
	 * so EOIR both drops priority and deactivates the source.
	 */
	gic_write32(gic_cpu_interface_base, GICC_CTLR, 1U);
	__asm__ volatile ("dsb sy\n\tisb" ::: "memory");
	bcm2712_gic_validate_cpu();
	ml_install_interrupt_handler(NULL, 0, NULL, bcm2712_gic_handle_irq, NULL);
}

void
bcm2712_gic_send_ipi(uint32_t physical_cpu)
{
	int logical_cpu = ml_get_cpu_number(physical_cpu);
	if (logical_cpu < 0 || logical_cpu >= BCM2712_GIC_MAX_CPUS) {
		panic("BCM2712: invalid IPI target %u", physical_cpu);
	}
	uint32_t target = os_atomic_load(&gic_targets[logical_cpu], acquire);
	if (!target) {
		panic("BCM2712: IPI to uninitialized CPU %u", physical_cpu);
	}
	gic_sync();
	gic_write32(gic_distributor_base, GICD_SGIR,
	    (target << 16) | (1U << 15) | BCM2712_GIC_IPI);
	__asm__ volatile ("isb" ::: "memory");
}

void
bcm2712_gic_init(vm_offset_t distributor_base, vm_offset_t cpu_interface_base)
{
	...
	if (gic_intids > BCM2712_GIC_MAX_INTIDS) {
		gic_intids = BCM2712_GIC_MAX_INTIDS;
	}
	printf("BCM2712: GICD typer=0x%x iidr=0x%x GICC iidr=0x%x (%u INTIDs)\n",
	...
	/* The non-secure distributor view exposes EnableGrp1 as bit zero. */
	gic_write32(gic_distributor_base, GICD_CTLR, 1U);
	gic_sync();
}

static int64_t
bcm2712_psci_call(uint64_t function, uint64_t arg0, uint64_t arg1,
    uint64_t arg2)
{
...
int64_t
bcm2712_psci_cpu_on(uint64_t physical_cpu, uint64_t entry, uint64_t context)
{
	return bcm2712_psci_call(PSCI_CPU_ON_64, physical_cpu, entry, context);
}

int64_t
bcm2712_psci_system_reset(void)
{
	return bcm2712_psci_call(PSCI_SYSTEM_RESET, 0, 0, 0);
}

#if DEVELOPMENT || DEBUG
static void
bcm2712_irq_test_timer_done(timer_call_param_t arg, __unused timer_call_param_t unused)
{
...
#if DEVELOPMENT || DEBUG
void
bcm2712_irq_test_bind_cpu(int cpu)
{
